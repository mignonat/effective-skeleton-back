...
const getInFile = (key, locale) => {
    filePath = config.getAbsRootPath()+'/translations/'+locale+'.properties'
    try {
        properties = reader(filePath)
        result = properties.get(key)
        if (result) return result
    } catch(ex) {
        log.error('locales.getInFile : problem while reading file '+filePath)
    }
}
...
const exportsFn = {
    print : (key, locale) => {
        ...
        if (translations.hasOwnProperty(key))
            ...
            var translation = getInFile(key, locale)
            if (!translation)
